...
class ObjectViewed(HashableModel):
    ...
    class Meta:
        indexes = [
            models.Index(fields=['user','object_id', 'content_type','content_object']),  
        ]
    def __str__(self):
...
class UserSession(HashableModel):
    user                = models.ForeignKey(User, blank=True, null=True,on_delete=models.CASCADE) # User instance instance.id
    ip_address          = models.CharField(max_length=220, blank=True, null=True) #IP Field
    ...
    class Meta:
        indexes = [
            models.Index(fields=['user','ip_address', 'session_key','active']),  
        ]
